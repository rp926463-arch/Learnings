...
def get_target_name(source_fld_name: String): String = {
  var res = ""
    for (lst <- parsedJson("mappings"))
      if (lst("source")("name") == source_fld_name)
        res = lst("target")("name")
  res
}
get_target_name("language")
...
for (i <- dfFromRDD1.columns)
  get_target_name(i)

// COMMAND ----------

parsedJson("mappings").foreach{ lst => lst("target")("name")}

// COMMAND ----------

//var source_fld_name = "language"
...
print(get_target_name("language"))

// COMMAND ----------

for (i <- dfFromRDD1.columns)
  println(get_target_name(i))

// COMMAND ----------

//dfFromRDD1.columns.foreach{ clmn => dfFromRDD1.withColumnRenamed(clmn,get_target_name(clmn)).show()}

//dfFromRDD1.select("*").withColumnRenamed("language","new_language").show

// COMMAND ----------


//CRETEA MAP from Mapping
